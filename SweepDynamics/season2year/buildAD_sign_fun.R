#!/usr/bin/env Rscript
buildAD <- function(infile.prefix, cutoff, thresh_color, minyear,
                   maxyear, drucke, startingSeason, filter_sign, plotwidth, format){

    #infile.prefix <- "HA-15-22"
    #cutoff <- 0
    #thresh_color <- 0.5
    #minyear <- 2015
    #maxyear <- 2022
    ...
    mutations <- paste("yearOutput/intermediateFiles/", infile.prefix, ".mutations.txt", sep="")
    mutmap <- paste("yearOutput/intermediateFiles/", infile.prefix, ".subtreeMutationMap.txt", sep="")
    numiso <- paste("yearOutput/intermediateFiles/", infile.prefix, ".numIsolates.txt", sep="")
    filename <- paste("yearOutput/", infile.prefix, ".", format, sep="")
    sumyfile <- paste("yearOutput/", infile.prefix, "summary.txt", sep="")
    significance <- paste("yearOutput/significance/", infile.prefix, ".significance.txt", sep="")    
    pvalues <- paste("yearOutput/significance/", infile.prefix, ".results.pvalues_fishertest.txt", sep="")  
    ...
    tmp_years <- seq(minyear, maxyear+1, 1)

    # filter data to plot only significant positions
    if(filter_sign=="true") {
    ...
    plotYears <- scan('yearOutput/years.txt', character(), quote="")
...
#    pchVec <- rep(3, nrow(test.data))
    ...
    plot(d.years[d.years > cutoff], test.data[1, d.years > cutoff], type="n",
         col=colVec[1], ylim=c(0, 1), axes=FALSE, xlab="Season", ylab="Frequency",
...
#		}
    ...
    write.table(resultfile, file=paste("yearOutput/", infile.prefix,".results.txt", sep=""), sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
    write.table(frequencyfile, file=paste("yearOutput/", infile.prefix,".significant_frequencies.txt", sep=""), sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
